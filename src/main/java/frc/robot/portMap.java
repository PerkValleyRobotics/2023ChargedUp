package frc.robot;

public class portMap {

    //drive train
    public static final int CAN_left1 = 1;
    public static final int CAN_left2 = 2;
    public static final int CAN_right1 = 3;
    public static final int CAN_right2 = 4;

    //Arm
    public static final int CAN_susRotate = 13;//5
    public static final int CAN_tilt = 6; //6
    public static final int CAN_extending = 14; //7

    //Grabber
    public static final int CAN_leftIntake = 7; //8
    public static final int CAN_rightIntake = 5; //9


    //controllers
    public static final int joystick = 0;
    public static final int xbox = 1;
}
