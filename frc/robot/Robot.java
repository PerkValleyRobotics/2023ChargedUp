...
public class Robot extends TimedRobot {
  ...
  */
  ...
  public void teleopPeriodic() {
    ...
    else if (oi.getPOV() == 180) tiltsetpoint -= .3;
    ...
    else if(oi.getXboxButtonPress(2)) { //home position
      done = false;
      targetSetpoint = 0;
      extendsetpoint = -1;
      lastPosition = 2;
    ...
    //Fail safe for pid
    if (armyBoy.getTiltEncoder() < 0) cutPID = true;
    ...
    //Dropping too quickly by passing the range and never setting done to true
    if (lastPosition == 2 && armyBoy.getTiltEncoder() < 6) cutPID = true;

    //Updates pid for arm tilt and extend
    if (!cutPID && (targetSetpoint != 0 || armyBoy.getExtendEncoder() > -0.2))armyBoy.tilt(tiltsetpoint);
    if (!cutPID && (done || targetSetpoint == 0)) armyBoy.extend(extendsetpoint);
    ...
    //Controls intake
    if (oi.getJoystickButtonPress(2)) grab.intake(0.1);
    else if (oi.getJoystickButtonPress(1)) grab.intake(-0.15);
    else grab.intake(0);
